...
录制输出同时请求 HLS 和 MP4。MP4 用于课程回放，HLS 用作快速可播和容灾。录制最大空闲时间配置为 300 秒，允许教师刷新或短时断流；课程结束仍必须主动调用 stop，避免持续计费。
...
2. 保留旧 `/api/token` 一个发布周期，仅用于旧链接回退，不再由新页面调用。
3. 验证 RTC、屏幕共享和录制后，删除灵动课堂 bundle、补丁脚本、依赖和全局样式清理代码。
