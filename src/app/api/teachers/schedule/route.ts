...
export async function GET(request: NextRequest) {
  ...
  try {
    const [lessons, blocks] = await withDatabaseReadRetry(() =>
      Promise.all([
    ...
    return (
      ...
      NextResponse.json({ error: "Failed to load schedules" }, { status: 500 })
    );
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error("Failed to save teacher schedule block:", error);
    return (
      databaseUnavailableResponse(error) ||
      NextResponse.json({ error: "Failed to save teaching plan" }, { status: 500 })
    );
  }
}
